...
public class scr_mirror : scr_Reflective
{
    ...
    private void Interact(GameObject origin, Vector2 startPoint, RaycastHit2D raycastHit2D, Material laserMaterial, Dictionary<GameObject, int> dictionary, int currentReflectionStep)
    {
        ...
        if (raycastHit2D.collider.TryGetComponent<scr_BossFirefly_LaserReceiver>(out scr_BossFirefly_LaserReceiver bossFireflyReceiver))
        {
            bossFireflyReceiver.Enable(laserMaterial);
        }
    }
